...
namespace MoreMountains.FeedbacksForThirdParty
{
    ...
    public class MMFeedbackPaniniProjection_URP : MMFeedback
    {
        ...
        protected override void CustomPlayFeedback(Vector3 position, float attenuation = 1.0f)
        {
            if (Active)
            {
                MMPaniniProjectionShakeEvent_URP.Trigger(ShakeDistance, Duration, RemapDistanceZero, RemapDistanceOne, RelativeDistance, attenuation, Channel, ResetShakerValuesAfterShake, ResetTargetValuesAfterShake);
            }
        }
    }
}
